...
function playSound(name, options) {
    ...
    //    return
    //}
    //worklet.port.postMessage({ buffer: { op: 'play', name: name, ...options } })
}
...
let biggestShapeWidth
let biggestShapeHeight
...
let audioArrayBuffersPromise = Promise.all([
    'beep1.wav',
    'beep2.wav',
    'beep3.wav',
    'beep4.wav',
...
;(async () => {
    ...
    addEventListener('keydown', e => {
        ...
        if (e.code == 'KeyP')
            return playSound('beep2.wav', { pitch: Math.random() + 0.5 })
    ...
    addEventListener('mousedown', async () => {
        ...
        if (audioContext.audioWorklet) {
            await audioContext.audioWorklet.addModule('audio.mjs')
            worklet = new AudioWorkletNode(audioContext, 'audio')
    
            const audioArrayBuffers = await audioArrayBuffersPromise
            for (const buf of audioArrayBuffers) {
                const [arrayBuffer, name] = buf
                const audioBuffer = await audioContext.decodeAudioData(arrayBuffer)
                const bufs = Array(audioBuffer.numberOfChannels).fill(0).map((_, i) => audioBuffer.getChannelData(i))
                worklet.port.postMessage({ buffer: { op: 'upload', data: bufs, sampleRate: audioBuffer.sampleRate, duration: audioBuffer.duration, name } })
            }
            const osc = audioContext.createOscillator()
            osc.connect(worklet).connect(audioContext.destination)
            osc.start()
        }
