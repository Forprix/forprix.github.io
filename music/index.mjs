...
;(async () => {
    ...
    async function addTrackIfNeeded() {
        ...
        if (!needed || tracksEnded)
            return

        const trackEl = trackListEl.appendChild(createTrackComponent())

        let thisTrackIndex = trackListEl.children.length - 1
        
        const track = await requestTrack(trackListEl.children.length - 1) 
        if (!track) // Additionaly clear children
            return trackEl.remove()
